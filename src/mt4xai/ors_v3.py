...
def _dp_with_heaps_and_ranks(y: np.ndarray, err: np.ndarray, errL: np.ndarray, errR: np.ndarray, errA: np.ndarray,
                             alpha: float, beta: float, q: int) -> List[Stage1Candidate]:
    """Stage-1 DP with heaps+ranks and cost-consistent both-ends candidates."""
    n = err.shape[0]
    if n < 2:
...
def stage1_dp(y: np.ndarray, q: int, alpha: float, beta: float) -> List[Stage1Candidate]:
    ...
    return _dp_with_heaps_and_ranks(y, err, errL, errR, errA, alpha=alpha, beta=beta, q=q)
